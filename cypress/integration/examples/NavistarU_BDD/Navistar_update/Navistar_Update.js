...
When('user click on seRach buttoN icon',function()
{
    cy.wait(2000)
...
{
    cy.get('div[id="buttonGroupWrapper"]').eq(0).find('pebble-button[id="refresh"][title="Refresh"]').click({force:true})
     cy.wait(20000)
...
When('user click on "DownloaDOption" button and User able to see the Download "VendorITeam" Excel file',function()
{
  ...
  When('user click on "homE" button And User able to see the home page',function()
{
 ...
 When('user click on "QuiCkacTions" button',function()
{
    cy.contains("Quick Actions").click()
    cy.wait(6000)
});
 When('user click on "uploadeNtities" and User able to see the "uplOAd BuLk enTItiEs" screen',function()
{
    ccy.get("#actionItem").contains("Upload Entities").click()
    cy.wait(10000) 
});
When('user click on "UPloadfiLE" and selects the import VendorItem file',function()
{
    cy.get('pebble-bulk-file-upload[id="fileUpload"]').find('input[type="file"]').selectFile('C:\\Users\\AJadhav\\Abhi_Git_Revrsesand\\Chic-Fila\\cypress\\fixtures\\update1.xlsx',{force: true})
     cy.wait(6000)
});
 When('user click on "ShowtasK DetAilS" and user able to view the "EntitydatA impoRTs proCEssing" screen',function()
{
    cy.get('[title="Show task details"]').contains("Show task details").click({force:true})
    cy.wait(10000)
});
When('user click on "refRESH" button and Users until waiting for "ValidationStaTus" not changing into the "Completed" status',function()
{
 ...
 When('user click on "sUcCeSs" button and User able to view the "EntiTyDaTa iMPoRtS coMpLeTed" screen',function()
{
    cy.get('[class="btn-link status-link"]').contains("Success").click({force:true})
     cy.wait(30000)
});
When('user click on "PartnuMbEr" id and User able to view the "veNdorITEM" screen',function()
{
