...
describe('Navistar', ()=>
{
  ...
  // })
    ...
    //  Search option
     ...
     cy.wait(20000)
     cy.pause()
     cy.get('div[id="buttonGroupWrapper"][class="buttonGroupWrapper"]').each(($el, index, $list) => {
      //     // $el is a wrapped jQuery element
          console.log($el, index )
      //     cy.wrap($el).click({force:true})
        })
     
    //  .eq(0).find('pebble-button[id="toolbar_actions"][title="Download"]').click({force:true})
    ...
    //  cy.get('[d="m11.291 13.684-10.996-11.842c-0.63053-0.67902-0.18335-1.842 0.70879-1.842h21.991c0.89325 0 1.3404 1.163 0.7099 1.842l-10.996 11.842c-0.3924 0.42138-1.0263 0.42138-1.4176 0"]').eq(0).click({force:true})
     cy.wait(6000)
     cy.get('[id="simpleButton"][role="button"]').eq(22).click({force:true})
    //  .each(($el, index, $list) => {
      // $el is a wrapped jQuery element
      // console.log($el, index )
    ...
    // 
    // cy.get('div[id="buttonTextBox"][class="text-ellipsis"]').contains("Download All").click({force:true})
    cy.get('[id="simpleButton"][role="button"]').each(($el, index, $list) => {
            // $el is a wrapped jQuery element
            console.log($el, index )
            // cy.wrap($el).click({force:true})
          })
